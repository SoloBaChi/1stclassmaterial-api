...
//  }
 ...
 const records =  userResults.map(result => [result.grade,result.unitLoad]);
//  console.log(records)
//  const gradepointSystem =  userResults.map(user => (user.gradePoint * user.gradePoint));
 const gradepointSystem =  userResults.map(user => user.gradePoint).reduce((acc,val) => (acc + val), 0) / userResults.length;
 console.log(gradepointSystem) 
 const cgpa = await calculateGpResults(records,gradepointSystem);
 return res.status(200).json(new ResponseMessage("success",200,`Done calculating CGPA..!`,{
    cgpa
 }))

}
catch(err){
return res.status(400).json(new ResponseMessage("error",400,`${err.message || `Error Calculating CGPA`}`))
}
}
...
const getSingleResult = async(req,res) => {
    try{
       ...
       if(!results){
        return res.status(400).json(new ResponseMessage("error",400,`No result found for this account.!`,{
            data:results
        }))
       }

       return res.status(200).json(new ResponseMessage(`success`,200,`fetched results successfully`,{
...
const getAllResults = async(req,res) => {
    try{
        ...
        if(!results){
         return res.status(404).json(new ResponseMessage("error",404,`No result found.!`))
        }
...
// Create a Result
 const createResult = async(req,res) => {
    const {courseCode,courseTitle,unitLoad,grade,gradePoint,semester} = req.body;
    try{
        const { _id:userId} = req.user;
    ...
    return res.status(200).json(new ResponseMessage("success",200,`Result data successfully added`,{
     newResult
    }))
    }
    catch(err){
...
const deleteSingleResult = async(req,res) => {
    try{
     ...
     if(!deletedResult){
        return res.status(400).json(new ResponseMessage("error",400,`No result found for deletion..!`));
     }
     return res.status(204).json(new ResponseMessage("success",204,`successfully deleted result`,{
        data:null
     }))
    }
    catch(err){
        return res.status(500).json(new ResponseMessage("error",500,`Internal Server` ))
    }
}
...
const deleteAllResults = async(req,res) => {
    ...
    const {_id:userId} = req.user;
     ...
     if(!deletedResults){
        return res.status(400).json(new ResponseMessage("error",400,`deleted all results Successfully`))
     }
     return res.status(204).json(new ResponseMessage("success",204,`Successfully Deleted all Results`,{
        data:null
     }))
    }
    catch(err){ 
